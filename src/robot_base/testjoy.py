...
def tj_callback(data):
    ...
    twist.linear.y = data.buttons[3]
    twist.angular.z = data.axes[0]

    # record values to log file and screen
    rospy.loginfo("twist.linear.x: %f ; angular %f ;twist.linear.y: %f ;", twist.linear.x, twist.angular.z, twist.linear.y)

    # process joystick buttons
    if data.buttons[0] == 1:        # green button on xbox controller
       rospy.loginfo("green")
    if data.buttons[1] == 1:        # red button on xbox controller
       rospy.loginfo("red")
    if twist.linear.y == 1:
       rospy.loginfo("Bule")
        #move_circle()
